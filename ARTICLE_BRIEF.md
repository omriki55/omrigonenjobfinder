...
## 6. שיתוף בלי חשיפה אישית (חובה לפני פרסום)

המטרה: כל קורא יוכל להעתיק את הפרומפטים ולהריץ אפליקציה משלו — **בלי שהנתונים האישיים שלי ייחשפו**.

### מה להסיר מה-repo לפני שהופכים אותו ציבורי
| קובץ | למה |
|------|-----|
| `scored_jobs.json` | שמות חברות + סטטוסי ראיונות אישיים |
| `contacts.json` | פרטי אנשי קשר |
| `feedback.json` | היסטוריית פעולות |
| `config.json` | פרופיל אישי, מיקום, טלפון |

**איך:** `git rm --cached <file>` → להוסיף ל-`.gitignore` → להשאיר קבצי דוגמה ריקים (`config.example.json`, `scored_jobs.example.json`).
במאמר עצמו: לחשוף ארכיטקטורה + פרומפטים בלבד, לא דאטה.

---

## 7. האונבורדינג — "כל אחד מריץ /setup"

הזווית: האפליקציה לא "שלי" — היא **תבנית שמתאימה את עצמה למשתמש** דרך שיחת onboarding ב-Claude.

```
/setup → Claude מנהל שיחה:
  1. אילו תפקידים אתה מחפש?
  2. איפה אתה מוכן לעבוד? (מיקום / רימוט)
  3. מה החוזקות והניסיון שלך?
  4. העלה קורות חיים (PDF / TXT)
→ Claude מייצר config.json + profile.md אישי
→ מריץ סריקה ראשונה אוטומטית
```

**סריקה ידנית של LinkedIn ע"י Claude:** המשתמש מדביק URL של משרה → Claude קורא את ה-JD, מנקד התאמה, וכותב מכתב/פנייה מותאם. כל התהליך מנוהל מ-Claude של המשתמש עצמו — לא דרך שרת שלי.

---

## 8. עלויות וסוג מנוי (שקיפות למשתמש)

⚠️ **חשוב להבהיר במאמר:** צריך **Anthropic API key** (pay-as-you-go), לא Claude Pro.

| רכיב | עלות משוערת |
|------|--------------|
| סריקה יומית (3× ביום) | ~$0.15–0.30 ליום |
| חודש שלם של סריקות אוטומטיות | **~$5–10** |
| Claude Pro (claude.ai) — לשימוש ידני בלבד | $20/חודש |
| Claude API — לאוטומציה (מומלץ) | Pay-as-you-go, ללא מחויבות |

**המלצה לקורא:** "פתחו חשבון ב-console.anthropic.com, הוסיפו $10 קרדיט — זה מספיק לחודש+ של שימוש."

---

## 9. מה צריך להשלים ב-Cowork
- [ ] מספר טוקנים מדויק מ-Console
- [ ] להפוך את ה-repo לציבורי + ניקוי מידע רגיש (סעיף 6)
- [ ] הוספת `/setup` flow + קבצי `.example.json` (סעיף 7)
